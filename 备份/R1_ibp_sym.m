...
vec3Da1Corr = vec3Da1 - (I/P0) * OmegaP0Ref . (MIs /. {a -> 0, b1 -> 2, b2 -> 1});

basis3D = Join[
  ks * vec3Da0,
  ks * vec3Da1Corr
] /. repvar; (* Apply P0 -> -I k0 immediately *)
...
basis1D3D = (repdimplus2 /@ basis1D) /. repvar /. reppara2N;
MIdlog = Join[basis3D, basis1D3D] /. reppowerselection // Complement[#, {0}] & // symmetry // DeleteDuplicates;

(* 对 P0 的导数 *)
dk0Term[r_R1] := listcal[r, 2, 1, 1];
dk0[expr_] := Expand[expr] /. a_. r_R1 :> a*dk0Term[r];
...
DEk00 = (dk0[MIdlog] // id // symmetry) /. reppowerselection // Complement[#, {0}] &;
DEks0 = (dksSymbolic[MIdlog] // id // symmetry) /. reppowerselection // Complement[#, {0}] &;

(* dlog 变量与 R1 基底的等式 *)
IBPsetdlogbasis = Table[G["dlog", i] - MIdlog[[i]], {i, Length[MIdlog]}];
